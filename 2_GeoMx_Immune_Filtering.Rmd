...
# ## cluster res_immune
# km.out <- kmeans(res_immune$beta, centers = 3, nstart = 20, algorithm="Lloyd", iter.max=50)
# 
# combined_clusts_immune <- as.data.frame(as.factor(km.out[["cluster"]]))
# colnames(combined_clusts_immune) <- c("cluster")
# combined_clusts_immune$cluster <- sort(combined_clusts_immune$cluster)
# 
# # standardize across cell type NOT sample
# standardized_immune <- apply(t(res_immune$beta), 1, cal_z_score)
# # standardized_immune <- apply(t(res_immune$beta[rownames(combined_clusts_immune),]), 1, cal_z_score)
# 
# combined_cluster_immune <- pheatmap(t(standardized_immune), color = colorRampPalette(c("#F9FABC", "#E29E6E", "#C13F34"))(50), show_colnames=FALSE, cluster_cols = FALSE, cluster_rows = FALSE) # , annotation_col = combined_clusts_immune)
#                                    
#                                    # , show_rownames=FALSE, show_colnames=FALSE, clustering_method = "ward.D2",cluster_rows = FALSE, cluster_cols = TRUE, annotation_row = combined_clusts_immune, treeheight_row = 0)
```
...
load("src/Brain_AllenBrainAtlas.RData")
signif(profile_matrix[seq_len(3), seq_len(3)], 2)

# heatmap(sweep(as.matrix(profile_matrix), 1, apply(profile_matrix, 1, max), "/"),
#         labRow = NA, margins = c(10, 5))

res_brain <- runspatialdecon(object = immune_demoData,
                      norm_elt = "q_norm",
                      raw_elt = "exprs",
                      X = as.matrix(profile_matrix),
                      align_genes = TRUE)

res_brain_small <- res_brain$beta[,c("Microglia", "Endothelial", "L5.ET", "L5.6.IT.Car3", "L6b", "Astrocyte", "VLMC", "Oligodendrocyte", "L4.IT", "PAX6", "Pericyte")] 

## cluster res_immune
km.out <- kmeans(res_brain_small, centers = 3, nstart = 20, algorithm="Lloyd", iter.max=50)

combined_clusts_brain <- as.data.frame(as.factor(km.out[["cluster"]]))
colnames(combined_clusts_brain) <- c("cluster")
combined_clusts_brain$cluster <- sort(combined_clusts_brain$cluster)

standardized_brain <- apply(t(res_brain_small[rownames(combined_clusts_brain),]), 1, cal_z_score)

combined_cluster_brain <- pheatmap(t(standardized_brain), show_colnames=FALSE, cluster_cols = FALSE, annotation_col = combined_clusts_brain)
                                   
           
# png("figs/nopdx/deconvolution-brain-profile.png", width=10, height=10, units="in", res=300)
# heatmap(t(res_brain_small), cexCol = 0.5, cexRow = 0.7, margins = c(10,7))
# dev.off()
```
