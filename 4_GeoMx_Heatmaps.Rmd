...
```{r}
immune_cormat <- cor(immune_log, method="pearson")
immune_heatmap <- pheatmap(immune_cormat, show_rownames = FALSE, show_colnames = FALSE, clustering_method = "average", 
                           clustering_distance_cols = "correlation", clustering_distance_rows = "correlation", annotation_col = subset(immune_metadata, select=c(PATIENT_SAMPLE, `Overall.Subtype.Primary`, `HER2.Low.Positive.HER2.0.Subgroup.Status.PRIMARY`, SITE)))

# png("figs/nopdx/immune-sample-correlation-test.png", width=12, height=12, units = "in", res=300)
# print(immune_heatmap)
# dev.off()

tumor_cormat <- cor(tumor_log, method="pearson")
tumor_heatmap <- pheatmap(tumor_cormat, show_rownames = FALSE, show_colnames = FALSE, clustering_method = "average",
         clustering_distance_cols = "correlation", clustering_distance_rows = "correlation", annotation_col = subset(tumor_metadata, select=c( PATIENT_SAMPLE, `Overall.Subtype.Primary`, `HER2.Low.Positive.HER2.0.Subgroup.Status.PRIMARY`, SITE)))

# png("figs/nopdx/tumor-sample-correlation-test.png", width=14, height=14, units = "in", res=300)
# print(tumor_heatmap)
# dev.off()
```
...
immune_cluster <- pheatmap(standardized_immune, show_rownames=FALSE, show_colnames=FALSE, clustering_method = "ward.D2",
         cluster_rows = FALSE, cluster_cols = TRUE, annotation_col = subset(immune_metadata, select=c(SITE, PATIENT_SAMPLE, HER2.Low.Positive.HER2.0.Subgroup.Status.PRIMARY)),
         annotation_row = immune_clusts, treeheight_row = 0)

# png("figs/nopdx/immune-tnbc-gene-clustering.png", width=12, height=12, units = "in", res=300)
...
km.out <- kmeans(tumor_log_hm[tumor_goi,], centers = 5, nstart = 20, algorithm="Lloyd", iter.max=50)
...
tumor_cluster <- pheatmap(standardized_tumor, show_rownames=FALSE, show_colnames=FALSE, clustering_method = "ward.D2",
         cluster_rows = FALSE, cluster_cols = TRUE, annotation_col = subset(tumor_metadata, select=c(SITE, HER2.Low.Positive.HER2.0.Subgroup.Status.PRIMARY, PATIENT_SAMPLE)),
