...
er_genes <- c("ESR1", "TROP2", "MUC1", "ERBB3", "ERBB2", "CCND1", "XPB1", "GREB1", "CA12", "GATA3", "FOXA1")
```
...
colnames(immune_melted)[1:3] <- c("Gene", "Sample", "Value")
...
for(i in 1:length(unique(immune_melted$Gene))){
  ...
  fm <- lmer(Value ~ SITE + (1 | PATIENT_ID), data=immunetest)
...
tumor_melted <- melt(tumor_log %>% 
                       ...
                       rownames_to_column())
colnames(tumor_melted)[1:3] <- c("Gene", "Sample", "Value")
...
# ggsave("figs/nopdx/immune-brain-dotplot-her2pos-vs-hrpos.png", clusterProfiler::dotplot(gse_immune, showCategory=25), width=15, height=15, units="in")
...
# ggsave("figs/nopdx/tumor-brain-dotplot-her2pos-vs-hrpos.png", clusterProfiler::dotplot(gse_tumor, showCategory=25), width=15, height=15, units="in")
...
immune_lme$er_resp <- "NO"
immune_lme$er_resp[rownames(immune_lme) %in% er_genes] <- "Estrogen_Response"
...
immune_lme$delabel <- NA
immune_lme$delabel[immune_lme$er_resp == "Estrogen_Response"] <- rownames(immune_lme)[immune_lme$er_resp == "Estrogen_Response"]

p <- ggplot(data=immune_lme, aes(x=Log2FC, y=-log10(FDR), col=er_resp, label=delabel, alpha = factor(er_resp, levels = c("NO", "Estrogen_Response")))) +
        geom_point() + 
        theme_minimal() +
        geom_text_repel() +
...
print(p)
# ggsave("figs/nopdx/immune-er-response-volcano.png")
# print(p)
...
# for(i in 1:length(gse_tumor@result$core_enrichment)){
  ...
  tumor_lme$er_resp <- "NO"
  tumor_lme$er_resp[rownames(tumor_lme) %in% er_genes] <- "Estrogen_Response"
  ...
  tumor_lme$delabel <- NA
  tumor_lme$delabel[tumor_lme$er_resp == "Estrogen_Response"] <- rownames(tumor_lme)[tumor_lme$er_resp == "Estrogen_Response"]

  p <- ggplot(data=tumor_lme, aes(x=Log2FC, y=-log10(FDR), col=er_resp, label=delabel, alpha = factor(er_resp, levels = c("NO", "Estrogen_Response")))) +
        geom_point() + 
        theme_minimal() +
        geom_text_repel() +
        scale_color_manual(values=c("blue", "grey", "red")) +
  ...
  print(p)
  # ggsave("figs/nopdx/tumor-er-response-volcano.png")
