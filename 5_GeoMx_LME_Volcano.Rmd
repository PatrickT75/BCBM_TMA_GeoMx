...
for(i in 1:length(unique(tumor_melted$Gene))){
  ...
  fm <- lmer(Value ~ SITE + (1 | PATIENT_ID), data=tumortest)
  # if (i < 4){
...
# ggsave("figs/nopdx/immune-tnbc-dotplot-brain-vs-breast.png", clusterProfiler::dotplot(gse_immune, showCategory=25), width=15, height=15, units="in")
...
# ggsave("figs/nopdx/tumor-tnbc-dotplot-brain-vs-breast.png", clusterProfiler::dotplot(gse_tumor, showCategory=25), width=15, height=15, units="in")
...
for(i in 1:length(gse_tumor@result$core_enrichment)){
  ...
  p <- ggplot(data=tumor_lme, aes(x=Log2FC, y=-log10(FDR), col=gsea, label=delabel)) +
        geom_point() + 
        theme_minimal() +
        geom_text_repel() +
